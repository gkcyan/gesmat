<div>
    ...
    @if($isOpen)
        @include('livewire.table-contenu')
    @endif
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
            <div class="flex items-center flex-grow h-10 space-x-1">
               
                
                <div>
                    <button wire:click="create()" class="px-3 py-2 text-xs font-medium leading-4 tracking-wider text-gray-900 uppercase bg-yellow-200 border border-yellow-900 rounded-md hover:bg-yellow-500 focus:outline-none">
            ...
            <div class="flex items-center mx-32 space-x-1">
                ...
                <div class="flex rounded-lg shadow-sm w-96">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="block w-full pl-10 transition duration-150 ease-in-out rounded-md form-input bg-gray-50 focus:bg-white sm:text-sm sm:leading-5" placeholder="Rechercher dans {{ $this->searchableColumns()->map->label->join(', ') }}" />
            ...
            <div class="flex items-center space-x-1">
                ...
                <div x-data="{ init() {
                    ...
                    <button wire:click="export" class="flex items-center px-3 space-x-2 text-xs font-medium leading-4 tracking-wider text-green-500 uppercase bg-white border border-green-400 rounded-md hover:bg-green-200 focus:outline-none"><span>Excel</span>
                ...
                <div class="table min-w-full align-middle">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        @foreach($this->columns as $index => $column)
                            ...
                            <div class="relative items-center table-cell w-48 h-12 px-6 py-3 overflow-hidden text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase align-top border-b border-gray-200 bg-gray-50 focus:outline-none">
                                <div class="px-3 py-1 rounded @if(count($selected)) bg-orange-400 @else @endif text-white text-center">
                                    {{ count($selected) }}
                                </div>
                            </div>
                            @else
                    ...
                    <div class="table-row bg-blue-100 divide-x divide-blue-200">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                <div class="flex flex-col items-center w-32 h-full px-6 py-5 space-y-2 overflow-hidden text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase align-top bg-blue-200 border-b border-gray-200 focus:outline-none">
                                    <div>SELECT ALL</div>
                                    <div>
                    ...
                    @empty
                        <p class="p-3 text-lg text-teal-600">
                           Aucune donnée correspond à votre recherche
                        </p>
                    @endforelse
                </div>
            </div>
                    ...
                    @if($this->results[0])
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="block w-full py-2 pl-3 pr-10 mt-1 text-base leading-6 border-gray-300 form-select focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
                                ...
                                <option value="99999999">TOUT</option>
                            </select>
                        </div>
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{ $this->results->firstItem() }} à {{ $this->results->lastItem().'ième' }} donnée affichée sur
                            {{ $this->results->total() }}
                        </div>
                    @endif
                </div>

<!-- This example requires Tailwind CSS v2.0+ -->
<!--div class="py-12 bg-fixe" style="background-image: url({{-- asset('images/background_.png') --}})"-->
<div class="py-12 bg-gray-100">

  <div class="px-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
    
    
    <div class="mt-10">
      <dl class="space-y-10 md:space-y-0 md:grid md:grid-cols-2 md:gap-x-8 md:gap-y-10">
        <div class="flex bg-gray-50">
          <div class="flex-shrink-0">
            <div class="flex items-center justify-center w-12 h-12 text-white bg-indigo-500 rounded-md">
              <!-- Heroicon name: globe-alt -->
              <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
              </svg>
            </div>
          </div>
          <div class="ml-4">
            <dt class="text-lg font-medium leading-6 text-gray-900">
              Agréement
            </dt>
            <dd class="mt-2 text-base text-gray-900">
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Maiores impedit g elit. Maiores impeditg elit. Maiores impedit.
            </dd>
          </div>
        </div>

        <div class="flex">
          <div class="flex-shrink-0">
            <div class="flex items-center justify-center w-12 h-12 text-white bg-indigo-500 rounded-md">
              <!-- Heroicon name: scale -->
              <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" />
              </svg>
            </div>
          </div>
          <div class="ml-4">
            <dt class="text-lg font-medium leading-6 text-gray-900">
              Appel d'offre
            </dt>
            <dd class="mt-2 text-base text-gray-900">
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Maiores impedit .
            </dd>
          </div>
        </div>

        <div class="flex">
          <div class="flex-shrink-0">
            <div class="flex items-center justify-center w-12 h-12 text-white bg-indigo-500 rounded-md">
              <!-- Heroicon name: lightning-bolt -->
              <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
          </div>
          <div class="ml-4">
            <dt class="text-lg font-medium leading-6 text-gray-900">
              Prestations
            </dt>
            <dd class="mt-2 text-base text-gray-900">
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Maiores impedit g elit. Maiores impeditg elit. Maiores impedit.
            </dd>
          </div>
        </div>

        <div class="flex">
          <div class="flex-shrink-0">
            <div class="flex items-center justify-center w-12 h-12 text-white bg-indigo-500 rounded-md">
              <!-- Heroicon name: annotation -->
              <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
              </svg>
            </div>
          </div>
          <div class="ml-4">
            <dt class="text-lg font-medium leading-6 text-gray-900">
              Facturation
            </dt>
            <dd class="mt-2 text-base text-gray-900">
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Maiores impedit .
            </dd>
          </div>
        </div>
      </dl>
    </div>
  </div>
</div>
